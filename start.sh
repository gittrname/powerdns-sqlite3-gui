...
/usr/sbin/pdns_server --daemon=no --allow-recursion=172.17.0.0/24 --recursor=${RECURSOR} --disable-axfr=yes --local-address=0.0.0.0 --launch=gsqlite3 --webserver=yes --webserver-address=0.0.0.0 --webserver-port=8080 --webserver-password=${WEBPASSWD} --experimental-json-interface --experimental-api-key=${WEBPASSWD} "$@"
